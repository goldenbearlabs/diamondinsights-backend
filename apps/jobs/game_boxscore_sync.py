...
class GameBoxscoreSync(Job):
    ...
    def run(self, db_session):
        self._log_start(
            ...
            window_start=f"{self.window_start_month}-{self.window_start_day}",
            window_end=f"{self.window_end_month}-{self.window_end_day}",
            rerun_all_boxscores=self.rerun_all_boxscores,
        )
        self.execute(db_session)
        self._log_end()

    def execute(self, db_session):
        season_year, start_date, end_date = self._season_window()
        self.logger.info(
            "schedule window start_date=%s end_date=%s",
            start_date,
            end_date,
        )
        ...
        self.logger.info("games collected count=%s", len(games))
        ...
        self.logger.info("target games for boxscores count=%s", len(target_game_ids))
        ...
        for i in range(0, total_boxscore_games, BOXSCORE_BATCH_SIZE):
            ...
            if batch_player_ids:
                ...
                if missing:
                    ...
                    _, _, _ = self._upsert_people_from_people_payload(db_session, people)
                    db_session.commit()
            ...
            if filtered_boxscore_rows:
                _, _ = self._upsert_game_boxscores(db_session, filtered_boxscore_rows)
            ...
            db_session.commit()
        self.logger.info(
            "boxscores complete total_games=%s failed_games=%s",
            total_boxscore_games,
            failed_games,
        )
        ...
        if missing_boxscore_games:
            self.logger.warning("missing boxscores remaining=%s", len(missing_boxscore_games))
        ...
        for i in range(0, total_games, GAME_BATCH_SIZE):
            ...
            db_session.commit()
        self.logger.info(
            "play-by-play complete games_with_pbp=%s failed_pbp=%s",
            games_with_pbp,
            failed_pbp,
        )
    ...
    def _fetch_schedule(self, start_date: str, end_date: str) -> Dict[str, Any]:
        ...
        return self._api_client.get(url, params) or {}
    ...
    def _upsert_people_from_people_payload(
        ...
        for _, person in people_by_id.items():
            if person is None:
                failed += 1
                continue
